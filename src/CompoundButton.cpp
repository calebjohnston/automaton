...
Element CompoundDefaultTransform(EntryState params) {
	auto element = vbox({
		text(params.label + " 1"),
		text(params.label + " 2"),
		text(params.label + " 3")
...
ftxui::Component CompoundButton(ConstStringRef label,
...
//      const bool focused_or_hover = focused || mouse_hover_; // CJJ was here...
      const bool focused_or_hover = mouse_hover_; // CJJ was here...
...
//      auto element =
//          (option_->transform ? option_->transform : CompoundDefaultTransform)  //
//          (state); CJJ was here...
      return CompoundDefaultTransform(state) | AnimatedColorStyle() | focus_management | reflect(box_);
    }
